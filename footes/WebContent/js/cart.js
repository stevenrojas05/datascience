var data;
function getItems(){
    $.getJSON("http://localhost:8080/FoodTimeEcommerce/ws/cart/list", function(result){
    	data = result.data; 
    	$('#div_items').empty();
    	var total = 0;
...
function removeItem(product_id){
	...
	$.getJSON("http://localhost:8080/FoodTimeEcommerce/ws/cart/del/" + product_id, function(result){
		getItems();
		updateItemsCount();
	});
}
...
function updateCart(){
	...
	text = '{"data":[' + text + ']}';
	var posting = $.post("http://localhost:8080/FoodTimeEcommerce/ws/cart/update/", text);
...
function checkout(){
	$.getJSON("http://localhost:8080/FoodTimeEcommerce/ws/cart/checkout", function(result){
		$("#checkout").addClass("disabled");
		updateItemsCount();
	});
	window.location.href = "http://localhost:8080/FoodTimeEcommerce/purchase.jsp";
}


function addToCart(product_id){
	$.getJSON("http://localhost:8080/FoodTimeEcommerce/ws/cart/add/" + product_id, function(json) {
...
function updateItemsCount(){
	$.getJSON("http://localhost:8080/FoodTimeEcommerce/ws/cart/items", function(json) {
		var items = json.items;
		$("#shopping_cart").text(items);
	});
}